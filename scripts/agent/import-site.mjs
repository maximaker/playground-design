...
function extractor() {
  ...
  function styleOf(el) {
    ...
    for (const p of KEEP) {
      ...
      if (p === 'grid-template-columns' && /px/.test(v)) {
        // Resolved track sizes are this window's answer, not the design's.
        out.push(`grid-template-columns:repeat(${v.split(' ').length}, minmax(0, 1fr))`);
        continue;
      }
      out.push(`${p}:${v}`);
    }
  ...
  function walk(el, depth) {
    ...
    if (el.tagName === 'IMG' || el.tagName === 'PICTURE') {
      ...
      return `<div style="background-color:#e7ded1;border-radius:18px;aspect-ratio:${ratio};width:100%"></div>`;
    }
    const tag = TAGS.has(el.tagName.toLowerCase()) ? el.tagName.toLowerCase() : 'div';
    ...
    for (const n of el.childNodes) {
      ...
      } else if (n.nodeType === 1 && depth < 16) inner += walk(n, depth + 1);
    }
